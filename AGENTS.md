...
- **参数虚设**：每个参数是否真实影响了行为？→ 写一个不同输入产生不同输出的断言来证明。
- **方向假设**：算法是否假设了某个方向/状态？→ 先写反面 case 的测试，确保算法能拒绝或正确处理。
- **魔数过期**：是否有硬编码的日期/年份/版本号？→ 一律用 `datetime.now()` 动态推导。
- **不确定时的默认值**：未知状态返回什么？→ 宁可不通过（`False`/`None`）也不做乐观假设。
- **结构完整性**：是否只检查了数量而忽略了顺序/交替性？→ 先验证结构约束，再下结论。

> 来源：Karpathy Behavioral Guidelines + 本仓库 5 次 code review bug 的根因归纳。
